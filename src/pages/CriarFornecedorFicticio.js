...
const CriarFornecedorFicticio = () => {
    ...
    const yupObject = {
        ...
        sobrenome: yup.string().required("Campo obrigatório").min(2),
    ...
    const handleSubmit = (values) => {
        ...
        getCoordinates().then(async (data) => {
            ...
            if (data.location && data.finalAddress.length > 5 && perfilImage && segmentos.length != 0 && categorias.length != 0 && subcategorias.length != 0 && ((cpfIsValid.current || cnpjIsValidRef.current) && !(cpfIsValid.current && cnpjIsValidRef.current)) && formaPagamento.length != 0 && horarioFuncionamento != 0 && descricaoLoja != 0) {
                ...
                newValues = { ...values, localizacao: JSON.stringify(data.location), endereco: data.finalAddress, cidade, segmentos: segmentos, categorias: categorias, subcategorias: subcategorias, cnpj: cnpjRef.current, tipoTel: tipoTel, imagem: perfilImage, cep, galeria, imagemFundo, formaPagamento: JSON.stringify(formaPagamento), descricaoLoja, dadosInteresse: JSON.stringify(dadosInteresse), statusConta: "usuario ficticio" }
                if (cpf) {
                    newValues.cnpj = null;
                    ...
                    navigate("/criar-fornecedor-ficticio")
                } else {
