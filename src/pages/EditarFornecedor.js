...
const EditarFornecedor = () => {
    ...
    const handleSubmit = (values) => {
        ...
        getCoordinates().then(async (data) => {
            let newValues
            if (data.location && cepValid.current && 5 && perfilImage && galeria.length != 0 && imagemFundo && segmentos.length != 0 && categorias.length != 0 && subcategorias.length != 0 && (cpfIsValid.current || cnpjIsValidRef.current) && formaPagamento.length != 0 && horarioFuncionamento != 0 && descricaoLoja != 0) {
                ...
                newValues = { ...values, nome: fornecedor.nome, sobrenome: fornecedor.sobrenome, email: fornecedor.email, id: fornecedor.pk_id, localizacao: JSON.stringify(data.location), endereco: data.finalAddress, cidade, segmentos: segmentos, categorias: categorias, subcategorias: subcategorias, cnpj: cnpj, tipoTel: tipoTel, imagem: perfilImage, cep, galeria, imagemFundo, formaPagamento: JSON.stringify(formaPagamento), descricaoLoja, dadosInteresse: JSON.stringify(dadosInteresse), statusConta: "ativo" }
                if (cpf) {
                    try {
                        newValues.cnpj = null;
